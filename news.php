...
{
$page = $_GET['page'];
$DATA_FILE = "files for txt/news.txt";
...
";
if ($stat == "Adminas"){
echo "<div class='input'><form action='news.php?id=irasau' method='post'>
...
echo"<div class=\"topbar\">";
  ...
  while ($viso_puslapiai < $viso_puslapiu)
        ...
        {
                echo"<a href=\"news.php?id=&amp;page=$starto_skaicius&amp;\">$starto_skaicius</a> ";
       }
        $viso_puslapiai++;
            $starto_skaicius++;
...
$bll = fopen("files for txt/news.txt","a");
fwrite($bll,"$new<br/>[$date]|\n");
fclose($bll);

$blla = fopen("files for txt/news_data.txt","w");
fwrite($blla,"$date");
fclose($blla);

$blla = fopen("files for txt/news_time.txt","w");
fwrite($blla,time());
fclose($blla);
}
